...
textFeedback:
  ...
  privacyPolicyUrl: https://your-site.example.com/privacy  # optional
  metrika:
    counterId: 12345678
    goals:
...
When `privacyPolicyUrl` is set, a personal data consent checkbox appears below the contact field. The submit button is disabled until the checkbox is ticked (only when the contact field is filled).
...
type TextFeedbackObject = {
    endpoint: string;
    privacyPolicyUrl?: string;  // URL shown in the personal data consent checkbox
    metrika?: {
        counterId: number;
        goals?: {
            button?: string;
