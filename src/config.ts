...
export type TextFeedbackObject = {
    ...
    /** URL of the privacy policy page shown in the personal data consent checkbox */
...
export function validateConfig(value: string | TextFeedbackObject | undefined): void {
    ...
    if (resolved.privacyPolicyUrl !== undefined) {
        ok(
            typeof resolved.privacyPolicyUrl === 'string' &&
                resolved.privacyPolicyUrl.trim() !== '',
            '[TextFeedback] textFeedback.privacyPolicyUrl must be a non-empty string',
        );
    }
}
